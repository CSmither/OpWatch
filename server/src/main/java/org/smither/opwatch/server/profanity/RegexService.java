...
public class RegexService {
    ...
    private void profAlert(Sign sign) {
        sendIRC(String.format("OPWATCH TRIGGERED!! Sign %s: \"%s, %s, %s, %s\" at %s %s %d,%d,%d placed by %s",
    ...
    private void profAlert(SignEvent signEvent) {
        sendIRC(String.format("OPWATCH TRIGGERED!! Event for sign %s: \"%s, %s, %s, %s\" at %s %s %d,%d,%d placed by %s",
                ...
                signEvent.getSign().getServer(), signEvent.getSign().getWorld(), signEvent.getSign().getX(), signEvent.getSign().getY(), signEvent.getSign().getZ(), signEvent.getSign().getPlacer()));
        log.info("! ! ! SIGN TRIGGERED OPWATCH ! ! !");
    }

    private void sendIRC(String string) {
        log.error("NO IRC IMPLEMENTED: " + string);
    }
